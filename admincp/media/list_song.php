...
if($mode == 'broken') {
	...
	$link_pages = "list_song.php?mode=broken&";
}
elseif($mode == 'top_hot') {
	$sql_where = "m_hot = 1";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?mode=top_hot&";
}
elseif($mode == 'hq') {
	$sql_where = "m_hq = 1";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?mode=hq&";
}
elseif($mode == 'mem_upload') {
	$sql_where = "m_mempost = 1";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?mode=mem_upload&";
}
elseif($mode == 'lyric') {
	$sql_where = "m_lyric_user = 1";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?mode=lyric&";
}
elseif($mode == 'clip_music') {
	$sql_where = "m_type = 2";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?mode=clip_music&";
}
elseif($mode == 'songs') {
	$sql_where = "m_type = 1";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?mode=songs&";
}
elseif($search) {
	$sql_where = "m_title_ascii LIKE '%".$search."%'";
	$sql_order = "ORDER BY m_id DESC";
	$link_pages = "list_song.php?search=".$search."&";
}
else {
	$sql_order = "m_id ORDER BY m_id DESC";
	$link_pages = "list_song.php?";
}
        ...
        <tr>
          <td valign="top" class="style_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                   ...
                   <input id=m_id size=10 value="" class="input">
                    ...
                    Tìm theo tên bài hát: <input id=search size=40 value="<? echo $search;?>" class="input">
                      <input type="image" src="../images/b_search.gif" onclick='window.location.href = "list_song.php?search="+document.getElementById("search").value;'>&nbsp;&nbsp;&nbsp; </td>
                  </tr>
                </table>               </td>
              </tr>
            </table>
		    ...
		    <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td colspan="">
                <form name=media_list method=post action='list_song.php' onSubmit="return check_checkbox();">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr >
...
if ($_POST['do']) {
	...
	if ($_POST['selected_option'] == 'del') {
		...
		mss ("Xóa thành công ","list_song.php");
	}		
	...
	elseif ($_POST['selected_option'] == 'normal') {
		...
		mss ("Update thành công ","list_song.php");
	}
	...
	elseif ($_POST['selected_option'] == 'set-top') {
		...
		mss ("Update thành công ","list_song.php?mode=top_hot");
	}
	...
	elseif ($_POST['selected_option'] == 'hq') {
		...
		mss ("Update thành công ","list_song.php?mode=hq");
	}
	exit();
}
?>